  ...
  updateMouseOverNodes(node2, e2) {
    ...
    for (const otherNode of nodes) {
      if (otherNode.mouseOver && node2 != otherNode) {
        this.pointer.resizeDirection = void 0;
    ...
    if (dragRect) {
      ...
      if (node2) {
        ...
        if (inputId === -1 && outputId === -1 && !overWidget) {
          this.pointer.resizeDirection = node2.findResizeDirection(e2.canvasX, e2.canvasY);
        } else {
          this.pointer.resizeDirection &&= void 0;
        }
      } else {
  ...
  deleteSelected() {
    ...
    for (const item of this.selectedItems) {
      if (item instanceof LGraphNode) {
        ...
        // node2.connectInputToOutput();  // DNNE: Disabled auto-rewiring on delete
        graph.remove(node2);
  ...
  drawConnections(ctx) {
    ...
    for (const node2 of nodes) {
      ...
      for (const [i, input] of inputs.entries()) {
        ...
        // DNNE Patch: Support multiple input connections
        // Check for links array first (multi-connection support), then fall back to single link
        const linkIds = input.links && input.links.length > 0 
          ? input.links 
          : (input.link != null ? [input.link] : []);
        
        if (linkIds.length === 0) continue;
        
        // Draw each connection for this input
